...
availableDistanceMetrics = {'sqeuclidean', 'euclidean', 'L1', 'cosine', 'emd', 'emdfast', 'EMDL1' 'chisq', ...
...
switch metric
  ...
  case 'emd'
    D = distEmd( X, Y );
  case 'emdfast'
    D = distEmdFast( X, Y, varargin{1}, varargin{2} );
  case 'EMDL1'
    D = emdL1(X, Y);
  otherwise
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function d = distEmdFast( X, Y, D, extra_mass_penalty)
m = size(X,1);
n = size(Y,1);
d = zeros(m,n);
for i=1:m
    for j=1:n
        d(i,j) = emd_hat_gd_metric_mex(X(i,:)',Y(j,:)', D, extra_mass_penalty);
    end
end
...
if 0 %d > 4000
    ...
    if gpuDeviceCount > 0
        ...
        mY = whos('Y');
        mD = whos('D');
        useGpu = gpu.FreeMemory > (mX.bytes + mY.bytes + mD.bytes);
    end
    ...
    for ii=1:n
        s = bsxfun(@plus, X, Y(ii,:));
        d = bsxfun(@minus, X, Y(ii,:));
        D(:,ii) = sum( d.^2 ./ (s+eps), 2);
    end
    D = D/2;    
