...
builtin('assert', ischar(mtype), 'sltoolbox:slmetric_pw:invalidarg', ...
    'The metric type should be a string.');

if strcmp(mtype, 'hamming') || strcmp(mtype, 'hamming_nrm')
    builtin('assert', (isnumeric(X1) || islogical(X1)) && ndims(X1) == 2 && ...
           (isnumeric(X2) || islogical(X2)) && ndims(X2) == 2, ...
        'sltoolbox:slmetric_pw:invalidarg', 'X1 and X2 should be numeric or logical matrices.');        
else
    builtin('assert', isnumeric(X1) && ndims(X1) == 2 && isnumeric(X2) && ndims(X2) == 2, ...
        'sltoolbox:slmetric_pw:invalidarg', 'X1 and X2 should be numeric matrices.');
end

builtin('assert', isa(X2, class(X1)), 'sltoolbox:slmetric_pw:invalidarg', 'X1 and X2 should be of the same class.');
    ...
    case {'hamming', 'hamming_nrm'}
        ...
        if islogical(X1) && islogical(X2)
            ...
            if isempty(varargin)
                ...
                t = varargin{1};
                builtin('assert', isnumeric(t) && isscalar(t), ...
